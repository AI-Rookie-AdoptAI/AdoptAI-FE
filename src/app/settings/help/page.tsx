...
const FAQ = [
  ...
  },
  {
    ...
    a: "네! 인스타그램, 당근 동네생활, 네이버 카페, 유기동물 공고 등 플랫폼별로 이미지 비율과 말투를 자동으로 맞춰드려요.",
  },
  {
    ...
    <div className="flex flex-col min-h-screen bg-surface-50">
      ...
      <main className="flex-1 overflow-y-auto scrollbar-hide px-5 pt-6 pb-10 flex flex-col gap-6">
        ...
        <section>
          ...
          <div className="flex flex-col gap-2">
            {FAQ.map((item, i) => (
              <div
                key={i}
                className="bg-surface-50 border border-brand-75 rounded-2xl overflow-hidden"
              >
                <button
        ...
        <section>
          ...
          <div className="bg-surface-50 border border-brand-75 rounded-2xl px-4 py-4 flex flex-col gap-3">
            <p className="text-[13.5px] text-brand-700 leading-relaxed">
              서비스 이용 중 문제가 생기거나 개선 사항이 있으면 알려주세요.
            </p>
            <a
              href="mailto:support@adoptai.kr"
              className="inline-flex items-center justify-center py-3 rounded-xl bg-brand-500 hover:bg-brand-600 transition-colors"
            >
        ...
        <Section title="정보">
          {LINKS.map(({ label, href, value }) => (
            <ListItem
              ...
              title={label}
              href={href}
              control={href ? { type: "chevron" } : { type: "value", text: value ?? "" }}
            />
          ))}
        </Section>
      </main>
