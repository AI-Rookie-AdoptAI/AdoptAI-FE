...
function ChatPageInner() {
  ...
  const statusMap: Record<string, { text: string; color: string }> = {
    ...
    draft_ready:{ text: "초안 준비됨",           color: "text-green-600" },
    editing:    { text: "수정 중",               color: "text-brand-400" },
    publishing: { text: "공고 파일 생성 중…",    color: "text-brand-300" },
    published:  { text: "파일 준비 완료",         color: "text-green-600" },
  };
  ...
  return (
    <div className="flex flex-col h-full min-h-screen bg-surface-50">
      <header className="flex items-center gap-2.5 px-[18px] pb-3 bg-surface-50 border-b border-brand-75 shrink-0" style={{ paddingTop: "max(3.25rem, calc(env(safe-area-inset-top) + 0.75rem))" }}>
        <Link
          href="/"
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-1.5">
            {(stage === "start" || stage === "draft_ready") && (
              <div className="w-1.5 h-1.5 rounded-full bg-confirmed-600 shrink-0" />
            )}
